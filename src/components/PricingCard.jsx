    ...
    >
      {isPopular && (
        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
          <span
            className="inline-flex items-center rounded-full bg-brand-primary px-3 py-1 text-xs font-medium text-brand-dark">
            Most Popular
          </span>
        </div>
      )}
      ...
      <div className="text-center">
        ...
        <div className="mb-4">
          ...
          {(price !== 'FREE' && price !== 'Contact Us') && <span className="text-white/60 ml-2">/month</span>}
        </div>
        <p className="text-white/70 mb-8 text-sm">{description}</p>

        <ul className="space-y-3 mb-8 text-left">
          {features.map((feature, idx) => {
            const isEverything = feature
              .toLowerCase()
              .includes('everything');
            return (
              <li key={idx} className="flex items-start text-sm">
                ...
                <span
                  className={`text-white/80 ${
                    isEverything ? 'font-extrabold text-white' : ''
                  }`}
                >
                  {feature}
                </span>
