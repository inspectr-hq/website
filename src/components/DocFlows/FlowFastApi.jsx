...
import serviceIcon from '../../assets/icons/fastapi.svg?react';
  ...
  const { nodes, edges } = useMemo(
    () =>
      buildBaseFlow({
        ...
        end: { label: 'FastAPI App', icon: serviceIcon, width: 140 }
      }),
    []
  );
...
export function FlowIngress() {
  const { nodes, edges } = useMemo(
    () =>
      buildIngressFlow({
        overrides: {'group_public': { label: 'fastapi-demo.in-spectr.dev'}},
        start: { label: 'Client', icon: clientIcon },
        end: { label: 'FastAPI App', icon: serviceIcon, width: 140 }
      }),
    []
