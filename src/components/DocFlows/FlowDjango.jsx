...
import serviceIcon from '../../assets/icons/python.svg?react';
  ...
  const { nodes, edges } = useMemo(
    () =>
      buildBaseFlow({
        ...
        end: { label: 'Django App', icon: serviceIcon, width: 140 }
      }),
    []
  );
...
export function FlowIngress() {
  const { nodes, edges } = useMemo(
    () =>
      buildIngressFlow({
        overrides: {'group_public': { label: 'django-demo.in-spectr.dev'}},
        start: { label: 'Client', icon: clientIcon },
        end: { label: 'Django App', icon: serviceIcon, width: 140 }
      }),
    []
