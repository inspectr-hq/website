...
<RelatedLink href="/docs/features/rules-engine">
  Learn how Rules can assign trace IDs
</RelatedLink>
...
**1) Simple, generic headers (quick start)**
...
**2) Manual override (Inspectr directive)**
...
<Aside type="note" title="When should I set my own trace ID?">
You usually **do not need to set a trace ID manually** — Inspectr automatically detects common tracing headers and correlation IDs.

Consider assigning your own trace ID when:
- You want to group requests by a **logical concept** (test run, eval execution, workflow id)
- Your system does not propagate tracing headers consistently
- You are correlating traffic across **tools or environments** that don’t share a tracing standard
- You want full control over trace boundaries in CI or automation scenarios

In these cases, assigning a trace ID via the **Rules Engine** is often the simplest approach.
</Aside>
...
### Inspecting local-to-remote development flows
Use tracing to understand how requests move from your local Inspectr proxy to a remote ingress or shared backend. This is especially useful when debugging issues that only occur in integrated environments.
