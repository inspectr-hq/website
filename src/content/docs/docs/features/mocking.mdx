...
<RelatedLink href="/docs/getting-started/mocking-quick-start/" title="Want a fast demo? Jump to the">Mocking Quick Start →</RelatedLink>
...
```bash
inspectr --mock-backend=https://inspectr.dev/demo/hello.openapi.yaml
```
...
<a
  href="https://inspectr.dev/launch?openapi=https://inspectr.dev/demo/auth-example.openapi.yaml"
  class="mb-4 inline-flex items-center justify-center rounded-xl border border-gray-800 px-4 py-2 text-sm font-semibold text-brand-primary transition-colors hover:border-brand-primary hover:text-brand-secondary"
>
...
See the guide [Response Overrides →](/docs/features/response-override/) for more examples
