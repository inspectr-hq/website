...
import inspectrGuardConfig from '../../../../assets/guides/settings-guard-config.png';
...
Customize token lifetime (default `24` hours) when starting Inspectr:

```bash
inspectr --auth-enabled=true --auth-secret=mysecret --auth-token-ttl=2
```

A fresh token is generated every run and expires after the time-to-live (TTL) you set.
...
- **When to use the token:** safer for public or shared channels because it expires per the time-to-live (TTL) you set.
...
```bash
inspectr --auth-enabled=true --auth-secret=mysecret --auth-token-ttl=24
```
...
<Image src={inspectrGuardConfig} alt="Inspectr - Guard Configuration" />

<Image src={inspectrGuard} alt="Inspectr - Guard tokens" />
