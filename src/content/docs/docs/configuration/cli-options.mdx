...
[//]: # (#| `--operations-eviction-policy` | string  | `drop_oldest` | Eviction policy used when the cap is exceeded. Current supported policy: `drop_oldest`.  |)
...
Run these commands with Administrator privileges on Windows or with `sudo` on Linux and macOS. See the complete [Background Services guide](/docs/features/background-services/) for named instances, logs, recovery, upgrades, and migration.
