...
Your private MCP server is now publicly available at:
...
Each entry in the UI shows the HTTP method, path, headers, body, and response. For MCP traffic, Inspectr also extracts tool, resource, prompt, session, and token information, giving you reachability plus protocol-aware observability.
