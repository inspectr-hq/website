---
title: Observability for MCP Servers
description: Track Model Context Protocol usage with Inspectr
---

import { FlowBase2 } from '../../../../components/DocFlows/FlowMcp.jsx';
...
[Inspectr](https://inspectr.dev) acts as a transparent proxy and tunnel for MCP traffic. Run it next to a local or private MCP server, point your MCP client at the Inspectr URL, and capture every request and response as the client interacts with your tools.

This is especially useful when your MCP server wraps internal systems such as private APIs, databases, knowledge bases, ticketing systems, or local development tools. Inspectr creates an outbound tunnel from your environment, so external MCP clients such as Anthropic Claude, OpenAI ChatGPT, and Microsoft Copilot can reach your server without inbound firewall rules or a public deployment.
...
- The MCP client targets the Inspectr tunnel URL, `https://mcp-demo.in-spectr.dev/mcp`, instead of the server directly.
...
Each entry in the UI shows the HTTP method, path, headers, body, and response. For MCP traffic, Inspectr also extracts tool, resource, prompt, session, and token information, giving you reachability plus protocol-aware observability. Filters help isolate calls from a particular tool or resource, and replay lets you reproduce behavior.

---

## Understanding Agent Behavior

<Image src={mcpTracingToolList} alt="Inspectr Tracing MCP server" />
...
<div class="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
  <a
    ...
    class="inline-flex items-center justify-center rounded-xl border border-gray-700 px-6 py-3 text-sm font-semibold transition-transform hover:-translate-y-0.5"
  >
    Expose an MCP server to Claude →
  </a>
  <a
    href="/docs/examples/expose-mcp-server-openai/"
    class="inline-flex items-center justify-center rounded-xl border border-gray-700 px-6 py-3 text-sm font-semibold transition-transform hover:-translate-y-0.5"
  >
    Expose an MCP server to OpenAI →
  </a>
</div>
