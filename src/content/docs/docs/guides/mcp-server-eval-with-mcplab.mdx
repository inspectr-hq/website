...
With MCPLab, you define eval scenarios that simulate real user interactions:
- tool calls
- prompt exchanges
- resource access
- multi-step flows
...
What eval tools intentionally do not provide by default is full traffic observability. They focus on outcomes, not wire-level inspection.

Eval results alone do not explain *why* something worked or failed.
...
## Step 3 — Create an eval config that targets Inspectr
...
When the eval is complete, stop Inspectr.
...
## Optional — Run evals through Inspectr Command Runner

For local automation or CI, you can let Inspectr start and stop the eval command for you.
